#!/usr/bin/env bash
# CI entrypoint to ensure ./gradlew exists and is executable, then execute it.
# Usage: bash ci-gradle.sh assembleDebug
...
# Ensure root-level gradlew exists
if [ ! -f "./gradlew" ]; then
  echo "[ci-gradle] Creating root-level gradlew shim..."
  cat > "./gradlew" << 'EOF'
#!/usr/bin/env bash
set -e
...
# Auto-create a minimal placeholder if missing
if [ ! -f "$WRAPPER" ]; then
...
echo "[ci-gradle] Executing ./gradlew $*"
exec ./gradlew "$@"
